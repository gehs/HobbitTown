...
def setup_lighting_ground():
    """Initialize ground effects lighting strip."""
    global pixels, is_available

    if not getattr(config, "ENABLE_LIGHTING", True):
        pixels = None
        is_available = False
        print("Ground Lighting: disabled (enable in config.py when the strip is connected)")
        return

    try:
        pixels = neopixel.NeoPixel(
            config.NEOPIXEL_GROUND_PIN,
            config.NUM_PIXELS_GROUND,
        ...
        pixels.show()
        is_available = True
        print("Ground Lighting: initialized")
    except Exception as exc:
        pixels = None
        is_available = False
        print(f"Ground Lighting: dry-load mode ({exc})")


def apply_lighting_preset_ground(preset_id):
    """Apply a preset color/animation to the ground effects strip."""
    ...
    if preset_id == 1:  # Morning - warm glow
        ...
        for i in range(config.NUM_PIXELS_GROUND):
            v = 0.784 - (0.784 - 0.251) * (i / (config.NUM_PIXELS_GROUND - 1))
        ...
        pass  # Handled in run_lighting_cycle_ground
    else:
        pixels.fill((0, 0, 0))
...
def set_all_lights_off_ground():
    """Turn off all ground effects pixels."""
    if pixels is None:
        return
    pixels.fill((0, 0, 0))
    pixels.show()
