"""
Sky Arc Lighting Controller for HobbitTown ESP32-S3.

Controls the sky arc strip (GPIO4): dawn (19px WS2812B) + noon (91px SK6812) + dusk (19px WS2812B).
Handles color presets, animations, and HSV conversions.
"""
...
def setup_lighting_sky():
    """Initialize sky arc lighting strip."""
    global pixels, is_available

    if not getattr(config, "ENABLE_LIGHTING", True):
        pixels = None
        is_available = False
        print("Sky Lighting: disabled (enable in config.py when the strip is connected)")
        return

    try:
        pixels = neopixel.NeoPixel(
            config.NEOPIXEL_SKY_PIN,
            config.NUM_PIXELS_SKY,
        ...
        pixels.show()
        is_available = True
        print("Sky Lighting: initialized")
    except Exception as exc:
        pixels = None
        is_available = False
        print(f"Sky Lighting: dry-load mode ({exc})")


def apply_lighting_preset_sky(preset_id):
    """Apply a preset color/animation to the sky strip."""
    ...
    if preset_id == 1:  # Morning - warm glow
        ...
        for i in range(config.NUM_PIXELS_SKY):
            v = 0.784 - (0.784 - 0.251) * (i / (config.NUM_PIXELS_SKY - 1))
        ...
        pass  # Handled in run_lighting_cycle_sky
    else:
        pixels.fill((0, 0, 0))
...
def set_all_lights_off_sky():
    """Turn off all sky arc pixels."""
    if pixels is None:
        return
    pixels.fill((0, 0, 0))
    pixels.show()
