...
class Smial1ModuleTest:
    ...
    def update(self):
        ...
        if not self._played and elapsed >= 5.0:
            self._played = True
            common.play_track_checked(1, self.track, loop=False)

        if elapsed < 7.5:
            level = min(1.0, max(0.0, (elapsed - 4.0) / 3.5))
            common.set_ground_segments(self._segment_map, ("smial_1",), (int(255 * level), int(170 * level), int(40 * level)))
        else:
            common.set_ground_segments(self._segment_map, ("smial_1",), (0, 0, 0))
